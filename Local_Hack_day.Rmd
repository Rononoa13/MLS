...
## Histogram
```{r}
qplot(data = good_country, x = Rank, binwidth = 0.1)
```
```{r}
...
```{r}
# distribution
ggplot(data = good_country, aes(Country)) + geom_density()
```
...
```{r}
library(corrgram)
corrgram(good_country, col.regions = colorRampPalette(c("red", "salmon",
  "white", "royalblue", "navy")))
```
